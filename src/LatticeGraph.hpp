...
class LatticeGraph
    : public Graph<GRAPH_PROPERTIES::lattice_graph_properties, GRAPH_PROPERTIES::lattice_vertex_properties,
                   GRAPH_PROPERTIES::lattice_edge_properties>
{
  public:
    using directions_type = std::vector<std::vector<std::vector<int>>>;

    LatticeGraph(const std::string &_name = "Honeycomb", const std::vector<std::size_t> &_grid_shape = {4, 4},
    ...
    void m_lattice_initialization()
    {
        ...
        for (auto &_vertex : this->vertices)
        {
            if (!_vertex.second.boundary)
            {
                ...
                auto _dirs = m_directions[std::reduce(_origin_co.begin(), _origin_co.end()) % m_directions.size()];
                ...
                for (auto _dir : _dirs)
                {
                    ...
                    if (this->adjacency_list[_target_id].find(_origin_id) == this->adjacency_list[_target_id].end())
                    {
                        // add edge
