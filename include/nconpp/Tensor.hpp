...
#ifndef INCLUDE_NCONPP_TENSOR_HPP_
#define INCLUDE_NCONPP_TENSOR_HPP_
...
// expand_dims
template <typename T>
static inline auto expand_dims(const expression_type<T> &M, std::size_t axis) {
    auto &&dM = M.derived_cast();
...
// ravel_index
template <typename T>
static inline auto ravel_index(const T &multi_index, const shape_type &shape) {
    return xt::ravel_index(multi_index, shape, xt::layout_type::row_major);
}

// ravel_index
template <typename T>
static inline auto ravel_index(const T &multi_index, const std::vector<std::size_t> &shape) {
    return xt::ravel_index(multi_index, shape, xt::layout_type::row_major);
}
...
static inline auto rand(shape_type shape, T lower = 0, T upper = 1,
...
// trace
template <typename T>
static inline auto trace(const expression_type<T> &M, int offset = 0, std::size_t axis1 = 0,
                         std::size_t axis2 = 1) {
...
// svd
template <typename T>
static inline auto svd(const expression_type<T> &M, bool full_matrices = true,
...
// outer, workaround
template <typename T>
static inline auto outer(const expression_type<T> &M, const expression_type<T> &W) {
    auto dM = M.derived_cast();
...
#endif // INCLUDE_NCONPP_TENSOR_HPP_
