...
class TensorNetwork
    : public Graph<GRAPH_PROPERTIES::tensornetwork_graph_properties,
...
private:
   ...
   */
  void
  create_edges(std::unordered_map<std::size_t, std::size_t> &_vertex_leg_map,
      ...
      if (_leg_index < 0) {
        if (m_dangling_legs.contains(_leg_index)) {
          throw std::invalid_argument(
              ErrorMessages::ERROR_CONSTRAINT_UNIQUELEGS);
        }
...
public:
   ...
   *      - contractible legs have the same positive integer as identifier,
   * hence occurring in pairs
   *      - legs with negative integers won't be contracted, so-called dangling
   * legs
   */
  explicit TensorNetwork(
      const std::vector<npp::tensor_type<T>> &tensor_list,
    ...
    for (std::size_t _vertex_index = 0; _vertex_index < tensor_list.size();
         _vertex_index++) {
      const std::vector<int> &_subscript_vector =
          subscript_vector_list[_vertex_index];

      create_edges(_vertex_leg_map, _vertex_index, _subscript_vector);

      this->vertices[_vertex_index].legs = std::move(_subscript_vector);
      this->vertices[_vertex_index].tensor =
          std::move(tensor_list[_vertex_index]);
    }
  }
  ...
  explicit TensorNetwork(std::vector<npp::tensor_type<T>> &&tensor_list,
    ...
    for (std::size_t _vertex_index = 0; _vertex_index < tensor_list.size();
      ...
      this->vertices[_vertex_index].legs =
          std::move(subscript_vector_list[_vertex_index]);
      this->vertices[_vertex_index].tensor =
          std::move(tensor_list[_vertex_index]);
    }
  }
   ...
   */
  const std::set<int> &DanglingLegs() { return m_dangling_legs; }

  /**
   * @brief Retrieve current legs (positive indices).
   *
   */
  const std::set<int> &Legs() { return m_legs; }
  ...
  void contract(std::vector<int> contraction_sequence = {},
    ...
    if (final_order.empty()) {
      final_order.insert(final_order.end(), m_dangling_legs.begin(),
                         m_dangling_legs.end());
    }
    ...
    while (!contraction_sequence.empty()) {
      ...
      if (_src == _dest) { // trace
        ...
        _src_tensor =
            npp::linalg::tensordot(_src_tensor, _dest_tensor, _axes_a, _axes_b);
   ...
   * @return std::size_t
   */
  std::size_t NumTensors() { return this->NumVertices(); }
  ...
  /**
   * @brief Split a vertex on a specific leg. The position to split is between
   * leg_index and leg_index + 1.
   *
   * @param leg_index
   * @param vertex_index
   */
  void split(std::size_t vertex_index, std::size_t leg_index) {
    ...
    // split legs
    std::vector<int> _left_legs(this->vertices[vertex_index].legs.begin(),
                                this->vertices[vertex_index].legs.begin() +
                                    leg_index);
    std::vector<int> _right_legs(this->vertices[vertex_index].legs.begin() +
                                     leg_index,
                                 this->vertices[vertex_index].legs.end());
  ...
  /**
   * @brief Connect all remaining vertices consecutive starting with the most
   * left one.
   *
   */
  void connect() {
  ...
  std::unordered_map<int, std::vector<int>> getLegs() {
    ...
    for (const auto &vertex : vs) {
      int vertexIndex =
          vertex.first; // Assuming vertex.first gives the vertex index
      const auto &legs =
          vertex.second
              .legs; // Assuming vertex.second.legs gives the legs of the vertex
      std::vector<int> legIndices;

      for (const auto &leg : legs) {
        legIndices.push_back(
            leg.index); // Assuming leg.index gives the index of the leg
      }
