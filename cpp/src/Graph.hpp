...
template <class G = GRAPH_PROPERTIES::default_struct,
          class V = GRAPH_PROPERTIES::default_struct,
...
public:
   ...
   *
   */
  std::size_t NumVertices() { return vertices.size(); }
...
#include <optional>
  ...
  std::size_t
  addVertex(std::optional<std::size_t> newVertexIndex = std::nullopt) {
    if (newVertexIndex) {
   ...
   *
   */
  std::size_t NumEdges() { return edges.size(); }
