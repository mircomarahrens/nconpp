...
class LatticeGraph : public Graph<GRAPH_PROPERTIES::lattice_graph_properties,
  ...
  public:
    ...
    LatticeGraph(const std::string &_name = "Honeycomb",
                 ...
                 const directions_type &_directions = {{{0, -1}, {0, +1}, {+1, 0}},
                                                       {{-1, 0}, {0, -1}, {0, +1}}},
    ...
    void removeEdgelessBoundaryVertices() {
        // Collect all vertices that participate in at least one edge
        std::set<std::size_t> connected;
        for (const auto &edge : this->edges) {
            connected.insert(edge.second.src);
            connected.insert(edge.second.dest);
        }

        // Collect boundary vertices with no edges
        std::vector<std::size_t> to_remove;
        for (const auto &vertex : this->vertices) {
            if (vertex.second.boundary && connected.find(vertex.first) == connected.end()) {
                to_remove.push_back(vertex.first);
            }
        }

        // Remove them — since they have no edges, we can erase directly
    ...
    void m_lattice_initialization() {
        ...
        for (auto &_vertex : this->vertices) {
            if (!_vertex.second.boundary) {
                ...
                auto _dirs = m_directions[std::reduce(_origin_co.begin(), _origin_co.end()) %
                                          m_directions.size()];
                ...
                for (auto _dir : _dirs) {
                    ...
                    std::size_t _target_id = npp::ravel_index(_target_co, m_boundary_grid_shape);
