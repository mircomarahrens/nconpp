...
static inline auto allclose(E1 &&e1, E2 &&e2, double rtol = 1e-05,
...
static inline auto prod(const shape_type &M) {
  ...
  return std::accumulate(M.begin(), M.end(), initialProduct,
...
// ravel_index
template <typename T>
static inline auto ravel_index(const T &multi_index,
...
static inline auto
dot(const expression_type<T> &xa,
...
// tensordot
template <typename T, typename O>
static inline auto tensordot(const expression_type<T> &xa,
                             const expression_type<O> &xb,
...
// trace
template <typename T>
static inline auto trace(const expression_type<T> &M, int offset = 0,
                         std::size_t axis1 = 0, std::size_t axis2 = 1) {
...
// transpose
template <typename T>
static inline auto transpose(const expression_type<T> &M,
...
// outer, workaround
template <typename T>
static inline auto outer(const expression_type<T> &M,
                         const expression_type<T> &W) {
  auto dM = M.derived_cast();
