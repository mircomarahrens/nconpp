...
from ..modules.mps import MPS
from ..modules.mpo import MPO
...
class TEBD(MPS, MPO):
    ...
    def init_unitaries_from_mpo(self):
        ...
        try:
            ...
            vL = np.zeros((1, self.mdim, 1), dtype=complex); vL[0,0,0] = 1.0
            vR = np.zeros((1, self.mdim, 1), dtype=complex); vR[0,self.mdim-1,0] = 1.0
            # iterate through iterable_list
            for iterables in self.lattice.iterable_list:
                for indices in iterables:
                    # indices
                    iWL = indices[1]; W0 = self.W_list[iWL]
                    iWR = indices[2]; W1 = self.W_list[iWR]
                    # set up unitaries
    ...
    def _checkrun(self):
        ...
        if None in vars(self).values():
            check = False
        if check == False: print("Simulation parameters are not ready.")
        return check
        ...
        if self._checkrun():
            ...
            if self.parallel==False:
                ...
                if self.nbp > mp.cpu_count():
                    print("You want to use more processes then cores available. This doesnt affect Pool(), but your number of processes is bounded by the max number of available cpus, setting nbp = mp.cpu_count().")
    ...
    def _update(self, result=None):
        ...
        """
        if result!=None: self.result = result
        # ... get indices, ...
        iWL = self.result[1]
        iWR = self.result[2]
