...
# TODO add indices to tensors in docstrings
...
# see arxiv:0804.2509 (McCulloch's iDMRG paper) for details.
def rotate_right(l, M):
    """========================================================================
    Rotates a singular value vector from the left to the right of an MPS
    matrix.
    ========================================================================
    Input: Vector l, MPS Matrix M
    Algorithm:
        1. lM = l M
        2. U S V = svd(lM)
        3. Mt = U, lR = SV
    Output: MPS Matrix Mt, Matrix lR
    ========================================================================
    Default Values: None
    ========================================================================
    """
...
def rotate_left(M, l):
    ...
    matrix.
    ========================================================================
    Input: MPS Matrix M, Vector l
    Algorithm:
        1. Ml = M l
        2. U S V = svd(Ml)
        3. lL = US, Mt = V
    Output: Matrix lL, MPS Matrix Mt
    ========================================================================
    Default Values: None
    ========================================================================
    """
...
def transfer_matrix(M1, M2):
    ...
    ========================================================================
    Input: Mps Matrix M1, Mps Matrix M2
    Algorithm:
        1. T = M1 M2
    Output: T
    ========================================================================
...
def construct_leftblock_transfer(A1, l1, B2, l2):
    ...
    ========================================================================
    Input: left Mps Matrix A1, singular vector l1, right Mps Matrix B2,
        singular vector l2
    Algorithm:
...
def construct_rightblock_transfer(l0, A1, l1, B2):
    ...
    ========================================================================
    Input: singular vector l0, left Mps Matrix A1, singular vector l1,
        right Mps Matrix B2
    Algorithm:
...
def calc_leftblock_eigvals(A_list, l_list, B_list, krylov_dim, eigvec=False):
    ...
    (if eigvec = True) of the left two-site block transfer matrices at all
    bonds.
    ========================================================================
    Input:
    Algorithm:
    ...
    ------------------+------------------+----------------------------------
    eigvec            | False (Default)  | Does not return the eigenvectors
                      | True             | return the eigenvectors
    ========================================================================
    """
    L = len(l_list)
    ei_list = [0] * L
    ...
    for index in range(L):
        ...
        if eigvec:
            ei, vi = arp.eigs(TL, k=krylov_dim, return_eigenvectors=eigvec)
            ei_list[i2] = ei
            vi_list[i2] = vi
        if not (eigvec):
            ei = arp.eigs(TL, k=krylov_dim, return_eigenvectors=eigvec)
            ei_list[i2] = ei
...
def calc_rightblock_eigvals(A_list, l_list, B_list, krylov_dim, eigvec=False):
    """========================================================================
    Calculates the eigenvalues and corresponding eigenvectors
    (if eigvec = True) of the right two-site block transfer matrices
    at all bonds.
    ========================================================================
    Input:
    Algorithm:
    ...
    for index in range(L):
        ...
        # diagonalizing transfer operator TL
        if eigvec:
            ei, vi = arp.eigs(TR, k=krylov_dim, return_eigenvectors=eigvec)
            ei_list[i0] = ei
            vi_list[i0] = vi
        if not (eigvec):
            ei, vi = arp.eigs(TR, k=krylov_dim, return_eigenvectors=eigvec)
...
def calc_norm(M_list, l_list, form="left"):
    ...
    Name              | Value            | Description
    ------------------+------------------+----------------------------------
    form              | "left" (default) | M_list is espected to be in
...
def calc_correlation_length(M_list, l_list, form="left", sites=2, ncv=40):
    ...
    eta = arp.eigs(T, k=2, which="LM", return_eigenvectors=False, ncv=ncv)
