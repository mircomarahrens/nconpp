...
class Eigensolver:
    ...
    """
    def __init__(self, lanczos_min=10, lanczos_max=50, lanczos_cutoff=np.finfo(float).eps*10,
            lanczos_reortho=False, num_vec=1, get_vec=False, which="ARPACK"):
        ...
        self.lanczos_reortho = lanczos_reortho   # set reorthogonalization during lanczos run
...
#        return(np.squeeze(np.tensordot(bra, ket, axes = ([0,3,1,2],[0,3,1,2]))))
    ...
    def _lanczos_hep(self, L0, W1, W2, R3, v0):
        ...
        Lanczos algorithm of hermitian eigenvalue problems for ground state
        and ground state energy calculation of tensor network states.
        ...
        """
        r = v0; E0 = 0.0
        alpha = []; beta = [np.linalg.norm(v0)]; Vj = []
        j = 0; conv = False
        while conv == False:
            ...
            if j>0: r -= Vj[j-1]*beta[j]                           # r = r - v_{j-1} b_{j}
            ...
            if j>1:
                ti, si = linalg.eig_banded([beta, alpha]) # pretty nice function, thx obama
    ...
    class _ham_mixed:
        ...
        contraction which is numerical cheaper then the direct diagonalization
        ...
        def __init__(self, L0, W1, W2, R3, dtype = float):
            ...
            self.shape = np.array([self.d**2*self.chi0*self.chi2,self.d**2*self.chi0*self.chi2])
            self.dtype = dtype
    ...
    def _arpack(self, L0, W1, W2, R3, v0, num_vec):
        ...
        if np.issubdtype(eta.dtype, np.complexfloating):
            eta = aslinearoperator(eta)
        ei, vi = arp.eigsh(eta, k=num_vec, which='SA', return_eigenvectors=True,
            v0=v0, ncv=self.lanczos_max, tol=self.lanczos_cutoff)
        return ei, vi
